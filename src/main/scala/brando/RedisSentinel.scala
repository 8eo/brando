...
class RedisSentinel(
...
) extends RedisConnectionSupervisor(database, auth, listeners, connectionTimeout, connectionHeartbeatDelay) {
  ...
  def disconnectedWithSentinel: Receive = {
    ...
    case SentinelConnect =>
      (sentinelClient ? Request("SENTINEL", "MASTER", master)) map { case Response.AsStrings(res) =>
  ...
  def isValidMaster: Future[Boolean] =
    (connection ? Request("INFO")) map { case Response.AsString(res) =>
