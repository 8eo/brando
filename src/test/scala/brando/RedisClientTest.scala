...
class RedisClientTest extends TestKit(ActorSystem("RedisClientTest")) with FunSpecLike with ImplicitSender {
  ...
  describe("smembers") {
    it("should return all members in a set") {
      ...
      assert(
        resp.getOrElse(List()).toSet ===
          Set(Some(ByteString("one")), Some(ByteString("two")), Some(ByteString("three")), Some(ByteString("four")))
      )
  ...
  describe("error reply") {
    it("should receive a failure with the redis error message") {
      ...
      expectMsgPF(5.seconds) { case Status.Failure(e) =>
  ...
  describe("select") {
    it("should execute commands on the selected database") {
      val brando = system.actorOf(Redis(Some(host), 6379, 5, listeners = Set(self)))
      expectMsg(Connecting(host, 6379))
      expectMsg(Connected(host, 6379))
  ...
  describe("multi/exec requests") {
    it("should support multi requests as an atomic transaction") {
      ...
      expectMsg(List(Some(Ok), Some(Queued), Some(Queued), Some(List(Some(Ok), Some(ByteString("somevalue"))))))
    }
  ...
  describe("blocking requests") {
    describe("subscribe") {
      ...
      it("should be able to subscribe to a pubsub channel") {
        ...
        expectMsg(Some(List(Some(ByteString("subscribe")), Some(ByteString(channel)), Some(1))))
      }
      ...
      it("should be able to unsubscribe from a pubsub channel") {
        ...
        expectMsg(Some(List(Some(ByteString("unsubscribe")), Some(ByteString(channel)), Some(0))))
  ...
  describe("blop") {
    it("should block then reply") {
      ...
      try {
        ...
        probePopRedis.expectMsg(Some(List(Some(ByteString("blpop:list")), Some(ByteString("blpop-value")))))
  ...
  describe("notifications") {
    it("should send a Connected event if connecting succeeds") {
      val probe = TestProbe()
      system.actorOf(Redis(Some(host), 6379, listeners = Set(probe.ref)))
    ...
    it("should send an ConnectionFailed event if connecting fails") {
      val probe = TestProbe()
      system.actorOf(Redis(Some(host), 13579, listeners = Set(probe.ref)))
    ...
    it("should send an AuthenticationFailed event if connecting succeeds but authentication fails") {
      val probe = TestProbe()
      system.actorOf(Redis(Some(host), 6379, auth = Some("not-the-auth"), listeners = Set(probe.ref)))
    ...
    it("should send a ConnectionFailed if redis is not responsive during connection") {
      ...
      val probe = TestProbe()
      system.actorOf(Redis(Some(host), port, listeners = Set(probe.ref)))

      probe.expectMsg(Connecting(host, port))
      probe.expectMsg(ConnectionFailed(host, port))
    }

    it("should send a notification to later added listener") {
      val probe  = TestProbe()
      val probe2 = TestProbe()
      val brando = system.actorOf(Redis(Some(host), 13579, listeners = Set(probe2.ref)))
      brando ! probe.ref

      probe.expectMsgAllOf(Connecting(host, 13579), ConnectionFailed(host, 13579))
      probe2.expectMsgAllOf(Connecting(host, 13579), ConnectionFailed(host, 13579))
    }

    it("should send a notification with the current status to later added listener") {
      val probe  = TestProbe()
      val probe2 = TestProbe()
      val brando = system.actorOf(Redis(Some(host), 6379, listeners = Set(probe2.ref)))
  ...
  describe("connection") {
    ...
    it("should try to reconnect if connectionRetryDelay and connectionRetryAttempts are defined") {
      ...
      val brando =
        TestActorRef(new Redis(host, 6379, 0, None, Set(listener.ref), 2.seconds, Some(1.seconds), Some(1), None))

      listener.expectMsg(Connecting(host, 6379))
      assert(brando.underlyingActor.retries === 0)
      listener.expectMsg(Connected(host, 6379))
      assert(brando.underlyingActor.retries === 0)

      brando ! Disconnected(host, 6379)

      listener.expectMsg(Disconnected(host, 6379))
      listener.expectMsg(Connecting(host, 6379))
    }

    it("should not try to reconnect if connectionRetryDelay and connectionRetryAttempts are not defined") {
      val listener = TestProbe()
      val brando   = TestActorRef(new Redis(host, 6379, 0, None, Set(listener.ref), 2.seconds, None, None, None))
    ...
    it("should not try to reconnect once the max retry attempts is reached") {
      ...
      val brando =
        TestActorRef(
          new Redis(host, 16379, 0, None, Set(listener.ref), 2.seconds, Some(1.seconds), Some(1), None)
        )

      listener.expectMsg(Connecting(host, 16379))
      assert(brando.underlyingActor.retries === 0)
      listener.expectMsg(ConnectionFailed(host, 16379))
  ...
  describe("eval") {
    it("should respond with result of operation") {
      ...
      brando ! Request("eval", "if ARGV[1] == \"hello\" then return 1 end", "0", "hello")
